...
# ── LangChain RAG — H03 트렌드 선행성 데이터 ─────────────────
# H03 EDA: 한국 K뷰티 성분 검색량은 미국보다 평균 25개월 선행 (CCF+Granger PASS)
try:
...
# H03 EDA 결과 — 성분별 한국→미국 선행 데이터 (서비스 연결용)
H03_DOCS = [
    "PDRN: 한국→미국 선행 25개월. US YoY 73.0%. US 진입기. 미국 주류화 예상 2028년 5월. Granger PASS(p<0.0001). 피부재생 핵심 성분. 선케어·스킨케어 적용.",
    "엑소좀 Exosome: 한국→미국 선행 34개월. US YoY 58.3%. US 진입기. 미국 주류화 예상 2029년 2월. K뷰티 얼리어답터 성분. 세포 재생·항노화.",
    "히알루론산 Hyaluronic Acid: 한국→미국 선행 28개월. US YoY 50.1%. US 진입기. 예상 2028년 7월. 보습 스킨케어 범용 성분.",
    "트라넥삼산 Tranexamic Acid: 한국→미국 선행 13개월. US YoY 48.8%. US 진입기. 예상 2027년 4월. Granger PASS(p=0.012). 브라이트닝·미백 세럼.",
    "나이아신아마이드 Niacinamide: 한국→미국 선행 38개월. US YoY 43.4%. US 진입기. 예상 2029년 5월. 스킨케어·선케어 범용. 모공·미백 다기능.",
    "EGF: 한국→미국 선행 36개월. US YoY 40.4%. US 진입기. 예상 2029년 3월. 세포 성장인자 고기능 성분. 안티에이징.",
    "레티놀 Retinol: 한국→미국 선행 11개월. US YoY 32.7%. US 진입기. 예상 2027년 2월. 안티에이징 스킨케어 핵심.",
    "NAD+: 한국→미국 선행 8개월. US YoY 21.1%. US 성장기. 예상 2026년 12월. Granger PASS(p=0.034). 에너지 대사 기능성 성분.",
    "글루타치온 Glutathione: 한국→미국 선행 32개월. US YoY 10.2%. US 성장기. 예상 2028년 12월. 미백·항산화 고기능 성분.",
    "무기자차 zinc oxide SPF mineral sunscreen 선케어: 미국 진입기 급성장. no white cast 백탁 없는 물리적 자외선차단 트렌드. K뷰티 선행 우위.",
    "세라마이드 ceramide barrier cream 스킨케어: 피부장벽 강화 barrier repair. 미국 진입기. K뷰티 배리어크림 글로벌 확산 중.",
    "K뷰티 한국 성분 미국 선행 평균 25개월. H03 CCF+Granger 분석 검증. 지금 한국에서 유행하는 성분이 약 2년 후 미국 주류화. 소싱 타이밍 지수.",
]

# 지연 초기화 — 첫 요청 시 1회만 로드
...
# RAG 프롬프트 — H03 컨텍스트 + 소싱 데이터 → 타이밍 인사이트
_SOURCING_PROMPT = PromptTemplate(
    input_variables=["context", "query", "n_products", "category", "complaints"],
    template="""당신은 BeautyBridge AI 소싱 어시스턴트입니다.
K뷰티 성분의 한국→미국 트렌드 선행성 데이터(평균 25개월 선행, H03 EDA 검증)를 기반으로 소싱 타이밍 인사이트를 제공합니다.
...
위 선행성 데이터를 활용해 선행 개월·US YoY·미국 주류화 예상 시점 등 구체적 수치를 포함한
2~3문장의 소싱 타이밍 인사이트를 한국어로 작성하세요.
...
def _chat_summary(query: str, n: int, category: Optional[str], complaints: list) -> str:
    ...
    if vs and llm and _SOURCING_PROMPT:
        try:
            ...
            chain   = LLMChain(llm=llm, prompt=_SOURCING_PROMPT)
            result  = chain.run(
                context    = context,
                query      = query,
                n_products = n,
                category   = category or "전체",
                complaints = c_str or "정보 없음",
            )
            return result.strip()
        except Exception:
            pass  # RAG 실패 시 2순위로

    # ── 2순위: OpenAI 직접 호출 (기존 방식) ────────────────────
    if _ai:
        try:
            prompt = (
                f'바이어 요청: "{query}"\n'
        ...
        _CAT_FALLBACK = {
            ...
            "클렌징": {
                ...
                "trends": [
                    ...
                    {"name": "Gentle Foam",     "growth": "+14%", "bar": 0.52, "hot": False},
                ],
            },
            "마스크": {
                "keywords": ["sheetmask", "centella", "brightening", "hydrating"],
                "trends": [
                    {"name": "Centella Mask",  "growth": "+35%", "bar": 0.90, "hot": True},
                    {"name": "Hydra Mask",     "growth": "+24%", "bar": 0.72, "hot": True},
                    {"name": "Brightening",    "growth": "+16%", "bar": 0.55, "hot": False},
                ],
            },
        }
        fb = _CAT_FALLBACK.get(cat_name or "", {
